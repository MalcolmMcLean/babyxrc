...
int bbx_options_error(BBX_Options *opt, char *errormesssage, int Nerror);
int bbx_options_Nargs(BBX_Options *opt);
char *bbx_options_opt_arg(BBX_Options *opt, int index);



static void checkunusedoptions(BBX_Options *opt);
static int parseoptionsv(BBX_Options *opt, int idx, char *fmt, va_list ret);
static void seterror(BBX_Options *opt, char *fmt, ...);
...
/*
  BBX_Options object constructor
...
BBX_Options *bbx_options(int argc, char **argv, char *flags)
{
    ...
    answer = malloc(sizeof(BBX_Options));
    if (!answer)
        goto error_exit;
    answer->argc = argc;
    answer->argv = 0;
 ...
 error_exit:
    bbx_options_kill(answer);
    return 0;
}

/*
  BBX_Options object destructor
 */
void bbx_options_kill(BBX_Options *opt)
{
    int i;
...
/*
  ...
  Params: opt - the BBX_Options object
          name - name of option
...
/*
  ...
  Params: opt - the BBX_Options object
          errormessage - buffer to receive error message
          Nerror - the sie of the errormessage buffer
  Returns: 0 if no error, 1 if errors
  Notes:
    Caller must parse every option with bbx_options_get()to prevent it being
    flagged as an unrecognised option.
    No need to check for null BBX_Option objects - code is null safe
    and bbX_options_error returns an out of memory error.
...
int bbx_options_error(BBX_Options *opt, char *errormessage, int Nerror)
{
    ...
    {
        if (errormessage)
            snprintf(errormessage, Nerror, "%s", "Out of memory");
        return -1;
    }
    checkunusedoptions(opt);
    
    if (opt->error && errormessage)
        snprintf(errormessage, Nerror, "%s", opt->errstr);
  ...
  /* this can happen if caller does not parse all flags */
    if (opt->firsttouched)
    {
        for (i=1;opt->argv[1][i];i++)
            if (!opt->firstused[i])
                seterror(opt, "Illegal flag -%c", opt->argv[1][i]);
    }
    ...
    if (N > 0 && N == opt->argc - 1 && opt->argv[1][0] == '-' && opt->flags)
    {
        for (i=1;opt->argv[1][i];i++)
            if (!strchr(opt->flags, opt->argv[1][i]))
                seterror(opt, "Unrecognised flag -%c", opt->argv[1][i]);
    }
        ...
        if (*ptr == '%')
        {
            ...
            switch (ptr[1])
            {
                case 's':
                    ...
                    {
                        seterror(opt, "Option %s expects an argument", opt->argv[idx]);
                        return 0;
                    }
                    ...
                    {
                        seterror(opt, "Option %s argument too long", opt->argv[idx]);
                        return 0;
                    }
                    break;
                case 'd':
                    ...
                    if (idx + answer + 1 >= opt->argc)
                    {
                        seterror(opt, "Option %s expects an integer argument", opt->argv[idx]);
                        return 0;
                    }
                    ...
                    {
                        seterror(opt, "Option %s integer out of range", opt->argv[idx]);
                        return 0;
                    }
                    *iptr = (int) ival;
                    if (*end)
                    {
                        seterror(opt, "Option %s must be an integer", opt->argv[idx]);
                        return 0;
                    }
                    break;
                case 'f':
                    ...
                    if (idx + answer + 1 >= opt->argc)
                    {
                        seterror(opt, "Option %s expects a numerical argument", opt->argv[idx]);
                        return 0;
                    }
                    ...
                    *fptr = strtod(opt->argv[idx + answer + 1], &end);
                    if (*end)
                    {
                        seterror(opt, "Option %s must be a number", opt->argv[idx]);
                        return 0;
                    }
                    break;
                default:
...
int xmain(int argc, char **argv)
{
    ...
    char errormessage[11024];

    opt = bbx_options(argc, argv, "-abc");
  
    strcpy(mess, "isdead");
    bbx_options_get(opt, "-fred -a", "%32s", mess);
    bbx_options_get(opt, "-age -AGE", "%d", &age);
    b = bbx_options_get(opt, "-b", 0);
